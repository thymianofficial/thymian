    ...
    if (isGroupedObservable<string, ThymianHttpTestTransaction>(curr)) {
      return curr.pipe(
        reduce(
          ...
          } satisfies HttpTest<HttpTestCase<[GroupedHttpTestCaseStep]>>
        )
      );
      ...
      } satisfies HttpTest<HttpTestCase<[SingleHttpTestCaseStep]>>);
    } else {
      return of({
        ctx,
        curr: {
      ...
      } satisfies HttpTest<HttpTestCase<[CustomHttpTestCaseStep<Input>]>>);
    }
  });
}
