...
<Aside type="note">
  `thymian report merge` reads its inputs from **CLI arguments only**: the
  `reports` and `specifications` keys of your configuration file are ignored
  (they belong to `thymian report convert`), so a config entry can never
  silently widen or alter an explicit merge. General configuration — plugins,
  formatters, log level — still applies.
</Aside>
...
<Aside type="caution">
  ...
  collapsed by its `runId` — but findings are never deduplicated across
  *different* runs: two separate tool runs over the same API double the summary
  counts, by design.
</Aside>
