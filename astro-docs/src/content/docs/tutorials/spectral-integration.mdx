...
## Combining Spectral Results with Other Reports

To fold converted Spectral results and existing Thymian reports into one consolidated document, see [Merging reports](/tutorials/report-merge/) — `thymian report merge` accepts the same typed `--report` inputs and converts external formats on the way in.
