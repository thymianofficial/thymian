...
A `ruleSets:` config entry follows this same contract — a package (bare specifier) or a local
path (see the [configuration schema](/references/configuration-schema/#rulesets)). A glob is not
a valid entry; to select many local files at once, bundle them into a rule set that globs them
via its [`pattern:` field](#globs).
...
entry is always one of the two kinds above — it resolves to a single module. That module may
then select its own member rule files with a glob, but the glob lives in the rule set's
`pattern:` field, not in the specifier you pass. See [Globs](#globs).
