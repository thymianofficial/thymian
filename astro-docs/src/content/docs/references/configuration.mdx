...
When a step needs exactly one match, point `reportsDir` at a directory that starts the run empty
(`reportsDir: build/reports` in a fresh workspace, or a step that clears it first).
