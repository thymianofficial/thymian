...
The [`init`](../../references/cli#thymian-init) command sets up Thymian in your project. The `--yes` flag will skip
...
After initialization, you'll have a `thymian.config.yaml` file that looks like this:
...
For more about HTTP conformance, see [here](../http-conformance). Besides the `error` severity level, there are also `warn` and `hint`, indicating a potential conformance issue and a suggestion for a truly optional mechanism of HTTP, respectively.
