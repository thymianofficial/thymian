...
/**
 * The `events` collection schema, in Astro's function form so the optional
 * `logo` field can use the content-collection `image()` helper (which is only
 * in scope here). NOTE: this is a `SchemaContext` FACTORY, not a plain Zod
 * schema — pass it to `defineCollection({ schema: eventsSchema })` (Astro
 * invokes it), and in tests resolve it once via `eventsSchema({ image })`
