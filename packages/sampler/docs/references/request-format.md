...
**Note:** Path parameters are replaced at runtime using the `pathParameters` field.
...
**Note:** Typically set to `false` for endpoints that create authentication resources (e.g., user registration).
...
**Note:** Values are substituted into the path before sending the request.
