...
// This security-relevant MUST guards against safe methods (e.g. GET) triggering
// unsafe side effects such as "page?do=delete". Whether a resource's *purpose*
// is to perform an unsafe action, and whether a safe method actually mutated
// state, is server-internal semantics: the URI and method alone cannot tell an
// action-bearing safe request from a benign one, and a successful response does
// not reveal a side effect. The conformant condition is therefore unobservable
// (a query-parameter heuristic would produce rampant false positives). The risk
// is real even though it cannot be mechanically validated.
