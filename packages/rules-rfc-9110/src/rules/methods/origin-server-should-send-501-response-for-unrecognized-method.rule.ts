...
// The SHOULD only applies when the server receives a method that is
// "unrecognized or not implemented". We cannot know which methods a deployed
// server recognizes, and Thymian generates requests from the spec (i.e. only
// methods the API declares), so it never naturally produces an unrecognized
