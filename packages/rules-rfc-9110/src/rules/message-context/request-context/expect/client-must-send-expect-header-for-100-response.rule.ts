...
export default httpRule(
  ...
  .severity('error')
  // Informational: the trigger ("a client that will wait for 100 (Continue)")
  // is client intent, not observable from the request; we cannot know a client
  // intended to wait, so there is no wire condition to check against.
  .type('informational')
