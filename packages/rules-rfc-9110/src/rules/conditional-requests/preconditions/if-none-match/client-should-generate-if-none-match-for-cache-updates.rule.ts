...
/**
 * This is a client-side SHOULD that is conditional on the client *desiring to
 * update stored responses that already carry entity tags* — i.e. it depends on
 * the client's private cache state, which is not present in the request and
 * which the framework cannot observe. There is no non-conformant request to
 * detect (omitting If-None-Match is fine when the client has no stored response
