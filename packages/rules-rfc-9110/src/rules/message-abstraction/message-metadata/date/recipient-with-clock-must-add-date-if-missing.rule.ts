...
// Informational (outcome 2): this MUST governs the internal behavior of a
// recipient with a clock (caches/forwards a response, adding Date if missing).
// It depends on internal recipient state (having a clock) and on what the
// recipient subsequently does downstream, neither of which is observable from a
// single recorded transaction, so there is no non-conformant condition to flag.
