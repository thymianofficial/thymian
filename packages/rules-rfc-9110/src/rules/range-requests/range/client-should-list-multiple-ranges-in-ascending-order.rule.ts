...
export default httpRule(
  ...
  .severity('warn')
  // Implementable (outcome 1): a request-side ("client SHOULD") rule, so it is
  // valid only in `analyze` — during `test` the request is generated by
  // Thymian, not a real client. It reads the Range request header VALUES on the
  // AnalyzeContext's validateHttpTransactions. appliesTo includes 'user-agent'
  // so it matches HAR-imported requests (HAR default request role).
  .type('analytics')
