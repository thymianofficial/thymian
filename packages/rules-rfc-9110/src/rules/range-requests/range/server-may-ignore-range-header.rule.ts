...
export default httpRule('rfc9110/server-may-ignore-range-header')
  .severity('hint')
  // A baseline "MAY" permission — a server is always free to ignore the Range
  // header (answering with a full 200). The "ought to support byte ranges" note is
  // non-normative advice, not a testable requirement.
  .type('informational')
