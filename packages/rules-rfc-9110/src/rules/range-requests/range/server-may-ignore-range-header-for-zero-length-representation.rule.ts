...
export default httpRule(
  ...
  .severity('hint')
  // Informational (outcome 2): a "MAY" permission. Ignoring the Range header for
  // a zero-length representation is allowed but not required, so there is no
  // non-conformant condition. (It also depends on the server-internal fact that
  // the selected representation has zero length, which is not reliably
  // observable.)
  .type('informational')
