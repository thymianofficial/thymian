...
// The fixture bar (ADR-0021 §4), on the corpus-load seam `profiles.test.ts`
// established: a declared context claims the assertion is *demonstrated*, so
// every rule the package ships carries one fixture per context it declares —
// no more, no fewer, and no exemptions. Each fixture then runs through the
// real engine of its context, where it must flag its violating input, pass
// its conforming one, and skip its undecidable one where it has one.

const contexts: FixtureContext[] = ['static', 'test', 'analytics'];
...
const skips = (results: RuleFnResult[]) =>
  results.filter((result) =>
    result.findings.some((finding) => finding.kind === 'rule-skip'),
  );

describe('the fixture bar', () => {
  it('carries one fixture module per executable rule the package ships', async () => {
...
describe.each(fixtures)('$rule.meta.name', (fixture) => {
  const declared = contexts.filter((context) => fixture[context] !== undefined);
  ...
  describe.each(declared)('in %s', (context) => {
    ...
    if (contextFixture.skips !== undefined) {
      ...
      it('skips, rather than passes, the input it cannot decide', async () => {
        ...
        expect(violations(results)).toEqual([]);
        expect(skips(results).length).toBeGreaterThan(0);
      });
    }
  });
});
