...
// A UA implementing only RFC 6797 ignores `preload` too, but it is sent on
// purpose — for the browser pre-load lists — and
// `server-should-send-sts-preload-directive` is the rule that speaks to it.
const DELIBERATE_DIRECTIVES = new Set(['preload']);

// The host-side contrapositive of a UA requirement: a directive RFC 6797 does
// not define is one a UA will skip. Heuristic in every context, because a
// directive defined after RFC 6797, or one some UAs recognise, is no defect.
