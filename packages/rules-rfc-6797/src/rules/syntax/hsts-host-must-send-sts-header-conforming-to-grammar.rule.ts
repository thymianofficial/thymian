...
import { stsValueRuleFns } from '../utils/sts-contexts.js';
...
// The one rule that owns a parse failure: every directive-level rule runs
// only on a value that parses, so a malformed value is reported once, here.
const { lint, live } = stsValueRuleFns(NAME, (fieldValue) => {
...
export default httpRule(NAME)
  ...
  .description(
    'An STS header field an HSTS Host includes in its response message MUST satisfy the grammar specified in Section 6.1, and UAs MUST ignore any STS header field containing directives, or other header field value data, that does not conform to the syntax defined in this specification.',
  )
  .summary(
    'HSTS Host must send a Strict-Transport-Security value that conforms to the RFC 6797 grammar.',
  )
  .explanation(
    'Browsers do not repair a malformed Strict-Transport-Security header: RFC 6797 requires them to ignore it entirely. A comma where a semicolon belongs, or a stray character in a directive, therefore switches HSTS off without any visible error, and the site stays open to protocol downgrade and cookie hijacking while its operators believe it is protected. Directives are separated by ";", and each is a token optionally followed by "=" and a token or a quoted string.',
  )
  .appliesTo('server')
