...
// mechanism and it is not used", which claims no non-conformance. The
// pre-load list the same paragraph permits is in no exchange, so a host that
// relies on it alone is still hinted at here.
export default httpRule(
  ...
  .explanation(
    'Returning a Strict-Transport-Security header over https is how a host tells browsers to use only https from then on; a response without it gives the browser nothing to note. The pre-load lists RFC 6797 also mentions are not visible in any exchange. The server-should-send-sts-header-over-secure-transport convention rule asks for the header outright and replaces this hint in the recommended profile.',
  )
  .appliesTo('server')
  .rule(checkPresence)
  .done();
