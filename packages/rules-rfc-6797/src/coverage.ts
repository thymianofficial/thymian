...
// qualify. Two of them are NOTEs in §7 that quote a keyword to explain one;
// the counting rule counts them, and their verdict is `nothing-to-check`.
// Three of §6.1's are addressed to the user agent; each is judged like any
// other unit, and two of them have a host-side contrapositive.
//
...
const units = {
  ...
  '9.2/1':
    'Effective request URIs are compared per RFC 2616 §3.2.3, except that an empty path MUST NOT be treated as equivalent to "/".',
} as const;
...
export default defineCoverage({
  ...
  rules: {
    ...
    // A convention rule, so no unit: 7.1/1 asks for the header only from a
    // host that has chosen to be an HSTS Host, and requiring it of every
    // response is the convention.
    ...
    // Its exact twin without the condition: a convention rule, so no unit.
