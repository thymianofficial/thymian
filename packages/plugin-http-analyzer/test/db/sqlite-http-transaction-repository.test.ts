...
describe('HttpTransactionRepository', () => {
  ...
  describe('readTransactionsByHttpFilter', () => {
    ...
    it('should filter by method', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(method('GET')),
      );
    ...
    it('should filter by path', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(path('/users')),
      );
      expect(results.length).toBe(2);
    ...
    it('should filter by status code', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(statusCode(200)),
      );
      expect(results.length).toBe(2);
    ...
    it('should filter by status code range', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(statusCodeRange(200, 299)),
      );
    ...
    it('should filter by request header', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(requestMediaType('application/json')),
      );
      expect(results.length).toBe(2);
      expect(
        results.every(
    ...
    it('should filter by origin', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(origin('https://api.example.com')),
      );
      expect(results.length).toBe(4);
      expect(
        results.every(
    ...
    it('should filter by hasBody (request)', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(hasRequestBody(true)),
      );
      expect(results.length).toBe(2);
    ...
    it('should support AND logic filter', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(and(method('GET'), path('/users'))),
      );
    ...
    it('should support OR logic filter', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(or(method('POST'), method('PUT'))),
      );
      expect(results.length).toBe(2);
      expect(
        results.every(
    ...
    it('should support NOT logic filter', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(not(method('GET'))),
      );
    ...
    it('should return empty iterator for no matches', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(method('PATCH')),
      );
    ...
    it('should handle constant filter (true)', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(constant(true)),
      );
    ...
    it('should handle constant filter (falsy)', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(constant('')),
      );
      expect(results.length).toBe(0);
    });
    ...
    it('should handle roles', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(method('DELETE'), ['proxy']),
      );
      expect(results.length).toBe(1);
    ...
    it('should handle origin wildcard', async () => {
      const results = Array.from(
        repo.readTransactionsByHttpFilter(
          matchesOrigin('https://api.*.com:8080'),
        ),
      );
  ...
  describe('readAndGroupTransactionsByHttpFilter', () => {
    ...
    it('should group transactions by method', async () => {
      const results = Array.from(
        repo.readAndGroupTransactionsByHttpFilter(
          statusCodeRange(200, 299),
          method(),
        ),
      );
    ...
    it('should handle complex filter with grouping', async () => {
      const results = Array.from(
        repo.readAndGroupTransactionsByHttpFilter(
          and(statusCodeRange(200, 499), not(statusCode(204))),
          and(origin(), path()),
        ),
      );
    ...
    it('should return empty iterator when filter matches nothing', async () => {
      const results = Array.from(
        repo.readAndGroupTransactionsByHttpFilter(method('PATCH'), method()),
      );
      expect(results.length).toBe(0);
    });
    ...
    it('should handle empty database', async () => {
      ...
      const results = Array.from(
        newRepo.readAndGroupTransactionsByHttpFilter(constant(true), method()),
      );
      expect(results.length).toBe(0);
      await newRepo.close();
    });
    ...
    it('should preserve transaction details in grouped results', async () => {
      const results = Array.from(
        repo.readAndGroupTransactionsByHttpFilter(constant(true), method()),
      );
      ...
      const trace1: CapturedTrace = [
        {
          ...
          response: {
            data: { statusCode: 200, headers: {}, trailers: {}, duration: 100 },
            meta: { role: 'server' },
          },
        },
      ];
      ...
      const trace2: CapturedTrace = [
        {
          ...
          response: {
            data: { statusCode: 201, headers: {}, trailers: {}, duration: 150 },
            meta: { role: 'server' },
          },
        },
      ];
      ...
      const trace3: CapturedTrace = [
        {
          ...
          response: {
            data: { statusCode: 200, headers: {}, trailers: {}, duration: 50 },
            meta: { role: 'proxy' },
          },
        },
        {
      ...
      const trace: CapturedTrace = [
        {
          ...
          response: {
            data: { statusCode: 204, headers: {}, trailers: {}, duration: 75 },
            meta: { role: 'origin server' },
          },
        },
      ];
